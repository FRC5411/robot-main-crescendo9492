...
public class SwerveController {
   ...
   *
   * @param cfg {@link SwerveControllerConfiguration} containing the PIDF variables for the heading
   *     PIDF.
   */
  ...
  /**
   * Helper function to get the {@link Translation2d} of the chassis speeds given the {@link
   * ChassisSpeeds}.
   *
   ...
   * disable a {@link SlewRateLimiter} set the desired one to null.
   *
   ...
   * @return Whether the values are within the deadband from {@link
   ...
   * @param yInput Y joystick input for the robot to move in the Y direction. Y = yInput * maxSpeed;
   ...
   * @param headingY Y joystick which controls the angle of the robot.
   * @param currentHeadingAngleRadians The current robot heading in radians.
   * @param maxSpeed Maximum speed of the drive motors in meters per second, multiplier of the
   *     xInput and yInput.
   * @return {@link ChassisSpeeds} which can be sent to th Swerve Drive.
   */
  public ChassisSpeeds getTargetSpeeds(
  ...
  /**
   * Get the {@link ChassisSpeeds} based of raw speeds desired in meters/second and heading in
   * radians.
   *
  ...
  public ChassisSpeeds getRawTargetSpeeds(
      ...
      double currentHeadingAngleRadians) {
    // Calculates an angular rate using a PIDController and the commanded angle. Returns a value
    // between -1 and 1
